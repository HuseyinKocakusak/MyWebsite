...
const researchOutputs = [
  {
    title: "Deciphering the Role of tRNA-Derived Fragments in Neurological and Psychiatric Disease Pathogenesis",
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-[#2F4F4F]/10 bg-background/90 backdrop-blur">
          ...
          <nav className="hidden items-center gap-2 md:flex">
            {navLinks.map((link) => (
              <button
                ...
                className="rounded-full px-3 py-2 text-sm font-semibold text-[#2F4F4F] transition duration-200 hover:scale-110 hover:bg-[#F0F8FF] hover:text-[#1f2f2f]"
              >
                {link.label}
              </button>
            ))}
        ...
        {mobileOpen ? (
          <div className="border-t border-[#2F4F4F]/10 bg-background/95 shadow-sm shadow-[#2F4F4F]/10 md:hidden">
            <div className="mx-auto flex max-w-6xl flex-col px-4 py-2">
              {navLinks.map((link) => (
                <button
                  key={link.id}
                  onClick={() => handleNav(link.id)}
                  className="w-full rounded-lg px-3 py-2 text-left text-sm font-semibold text-[#2F4F4F] hover:bg-[#F0F8FF]"
                >
                  {link.label}
                </button>
              ))}
      ...
      <main className="mx-auto max-w-6xl space-y-16 px-4 pb-16 pt-28 md:px-6">
        <section id="home" className="grid gap-8 rounded-2xl bg-white/70 p-6 shadow-sm shadow-[#2F4F4F]/10 md:grid-cols-[1.1fr,0.9fr]">
          <div className="space-y-4">
            <span className={badgeClasses}>
              <Icons.Sparkles className="h-4 w-4" /> Researcher • Beekeeper • Athlete
            </span>
            <h1 className="text-4xl font-bold leading-tight text-[#2F4F4F]">
              Crafting healthier systems across the brain, the hive, and the field.
            </h1>
            <p className="text-base text-[#333333]/85">{scienceFocusCopy}</p>
            <div className="flex flex-wrap gap-3">
              <button
              ...
              <button
                onClick={() => handleNav("science")}
                className="inline-flex items-center gap-2 rounded-full border border-[#2F4F4F]/30 bg-[#F0F8FF] px-5 py-3 text-sm font-semibold text-[#2F4F4F] transition hover:border-[#2F4F4F]/50"
              >
                View research
              </button>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl bg-[#2F4F4F] p-5 text-white shadow-lg shadow-[#2F4F4F]/30">
              <div className="flex items-center gap-3">
                <div className="rounded-xl bg-white/15 p-3">
                  <Icons.Brain className="h-6 w-6" />
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-[#F0F8FF]">Current Focus</p>
                  <p className="text-lg font-semibold">Microglial EV cargo</p>
                </div>
              </div>
              <p className="mt-3 text-sm text-[#F0F8FF]/90">Mapping how noncoding RNAs shape neuronal integrity.</p>
            </div>
            <div className="space-y-4">
              <div className="flex items-center gap-3 rounded-2xl bg-white/85 p-4 shadow-sm shadow-[#2F4F4F]/10">
                <div className="rounded-xl bg-[#F0F8FF] p-3 text-[#2F4F4F]">
                  <Icons.Hexagon className="h-6 w-6" />
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-secondary">Apiary Log</p>
                  <p className="text-sm text-[#333333]/85">Seasonal splits, health metrics, regenerative design.</p>
                </div>
              </div>
              <div className="flex items-center gap-3 rounded-2xl bg-white/85 p-4 shadow-sm shadow-[#2F4F4F]/10">
                <div className="rounded-xl bg-[#F0F8FF] p-3 text-[#2F4F4F]">
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-secondary">Athlete Notes</p>
                  <p className="text-sm text-[#333333]/85">Data-driven training, recovery, and team play.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        >
          <div className="grid gap-6 md:grid-cols-[220px,1fr]">
            ...
            <div className="space-y-4 text-[#333333]/85">
              <p>
                I navigate the intersections of science, ecology, and sport to create measurable impact. I love translating field
                observations into repeatable lab insights while keeping a beekeeper’s patience and an athlete’s discipline.
              </p>
